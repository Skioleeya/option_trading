...
class AppContainer:
    ...
    def __init__(self) -> None:
        ...
        self.atm_decay_tracker = AtmDecayTracker(self.redis_service.client, None)
    ...
    async def initialize_all(self) -> None:
        ...
        self.atm_decay_tracker.ctx = self.option_chain_builder._gateway.quote_ctx
    ...
    async def shutdown_all(self) -> None:
        ...
        if USE_L2 and self._l2_reactor and hasattr(self._l2_reactor, "_audit_writer"):
            self._l2_reactor._audit_writer.flush()
    ...
    async def _agent_runner_loop(self) -> None:
        ...
        """
        from l0_ingest.feeds.rate_limiter import longport_limiter
        next_tick = time.monotonic()
        ...
        while self._running:
            ...
            try:
                ...
                if USE_L2 and self._l1_reactor and self._l2_reactor:
                    ...
                    # Shim: Convert DecisionOutput → legacy dict for backup/display
                    ...
                    decision = result # from_decision_output handles dicts now
                ...
                # 4. Periodically flush L2 audit logs (e.g. every 60 computations)
                if USE_L2 and self._l2_reactor and hasattr(self._l2_reactor, "_audit_writer"):
                    if self._total_computations > 0 and self._total_computations % 60 == 0:
                        self._l2_reactor._audit_writer.flush()
