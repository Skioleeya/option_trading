...
- [ ] T2-1: Add `-Timezone` option in `scripts/new_session.ps1` (orig: P1)
- [ ] T2-2: Extend `validate_session.ps1 -Strict` (orig: P2)
- [ ] T2-3: Add CI hook / pre-merge job for session validation (orig: P2/Parking)
- [ ] T2-4: Harden workspace hygiene for generated runtime artifacts (orig: P2)
- [ ] T2-5: Add focused UI regression tests (DecisionEngine/Header/Debug hotkey paths).
...
- [ ] Mark 2 obsolete items in historical session files with `SUPERSEDED-BY: <session-id>` to prevent debt inflation.
