...
from shared.services.l0_runtime.source.runtime.quote_runtime import PythonQuoteRuntime, RustQuoteRuntime
...
async def test_rust_quote_runtime_decodes_rows_and_indexes(monkeypatch):
    ...
    monkeypatch.setattr(
        "shared.services.l0_runtime.source.runtime.quote_runtime.l0_rust",
    ...
    assert runtime.transport_contract() == {
        "transport": "arrow_ipc_named_event",
        "shm_path": "sentinel_shm_live_arrow",
        "signal_name": "sentinel_shm_live_arrow_signal",
    }
...
async def test_python_quote_runtime_maps_sdk_objects_to_contracts(monkeypatch):
    monkeypatch.setattr(
        "shared.services.l0_runtime.source.runtime.quote_runtime.MarketDataGateway",
        _FakeMarketDataGateway,
    )

    runtime = PythonQuoteRuntime(_DummyConfig())
    assert runtime.transport_contract() == {
        "transport": "python_event_queue",
        "shm_path": None,
        "signal_name": None,
    }
    option_quotes = await runtime.option_quote(["SPY260101C560000.US"])
