"""l2_decision.feature_store.store — Unified real-time feature registry and computation.
...
# ---------------------------------------------------------------------------
# Forward declaration to avoid circular import (EnrichedSnapshot is in l1_compute)
# We use TYPE_CHECKING + Any for runtime to stay dependency-independent.
# ---------------------------------------------------------------------------
try:
    from l1_compute.output.enriched_snapshot import EnrichedSnapshot  # type: ignore
    _HAS_L1 = True
except ImportError:
    _HAS_L1 = False
...
try:
    from l2_decision.events.decision_events import FeatureVector
except ImportError:
...
class FeatureStore:
    ...
    def compute_all(self, snapshot: Any) -> "FeatureVector":
        ...
        """
        from l2_decision.events.decision_events import FeatureVector  # lazy import
