...
#### Volume Imbalance Engine (`VolumeImbalanceEngine` — **Rust Powered**)
- **C/P Balance**: 计算当日 Call/Put 成交量不平衡度，输出 `{consensus, strength}`。
- **Acceleration Ratio (Burst Detection)**: 
  - **Interval Volume**: 基于两个 Tick 之间的 **Delta**（而非当日累积量）计算。保证系统能捕捉到秒级的爆发。
  - **Rust Kernel**: 使用 `ndm_rust.compute_vol_accel` 实现 decoupled EMA 基准，比率为 `delta / prev_ema`。识别爆发比率 $\ge 3.0$。
- **Squeeze Alert**: 当 $Ratio \ge 3.0$ 且 Net GEX 为负时，触发 `dealer_squeeze_alert = True`。
...
fused_confidence (DynamicWeightEngine 输出)
     ...
     │    └─ BULLISH 方向 → × 1.15
     │
     ├─ VOL ACCEL SQUEEZE (Accel Ratio >= 3.0)
     │    └─ ALL 方向 → × 1.25 (强制风险因子提权)
     │
...
| `services/analysis/volume_imbalance_engine.py` | C/P 量不平衡 + **Rust 加速加速比** |
