...
## 2. BSM 计算流程 (`GreeksEngine`)

为了避免 CPU 密集型的计算阻塞处理 WebSocket 更新的 `asyncio` 回调循环，`_enrich_chain_with_local_greeks()` 被抽取到新的 **`GreeksEngine`** 模块，并通过 `asyncio.to_thread()` 放置在纯后台线程中运行。
...
6. 订单流微观结构 (`DepthEngine` — **Rust Powered**)
   - **Toxicity Score (VPIN)**: 基于逐笔成交 (SubType.Trade) 计算方向性毒性。
     - **Rust Kernel**: 使用 `ndm_rust.update_vpin_logic` 高效处理高频成交流。
     - **Dynamic Alpha**: 衰减因子 $\alpha$ 根据单成交量 $v$ 动态调整：$\alpha = \min(1.0, v / \text{BucketSize})$。机构大单将瞬间重置评分。
     - **Neutral Dilution**: 长桥 `TradeDirection.Neutral (0)` 盘计入分母以稀释毒性，但赋予 $0.0$ 方向权重。
   - **BBO Imbalance**: 基于盘口深度 (SubType.Depth) 计算买卖挂单失衡。
     `imbalance = EWMA( (bid_qty - ask_qty) / (bid_qty + ask_qty) )`
```

**性能增强**: 核心计算逻辑从 Python 卸载至 Rust，消除大型期权链处理时的 Loop Overhead，确保微秒级响应。
...
| 文件 | 职责 |
|------|------|
| `services/analysis/greeks_engine.py` | `GreeksEngine.enrich()` — 离线 BSM 计算与单次聚合 |
