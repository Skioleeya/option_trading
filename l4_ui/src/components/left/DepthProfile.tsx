...
const DepthProfileRow: React.FC<{
    ...
    return (
        <div
            ...
            className={`relative group w-full h-[20px] flex items-center justify-center  ${isFocusZone ? 'bg-white/[0.03]' : 'hover:bg-white/[0.02]'} ${isNavHighlighted ? 'ring-1 ring-[#fbbf24]/80 bg-[#fbbf24]/10' : ''}`}>
            ...
            {row.is_spot && <div className="absolute top-[50%] left-0 w-full h-[1px] border-b border-dashed border-[#ff3366]/60 z-20 pointer-events-none"></div>}
            ...
            <div className="flex items-center w-full h-full relative z-10 box-border px-4">
                ...
                <div className="flex-1 h-full flex justify-end items-center relative">
                    {hasPut && (
                        <div className={`h-[12px] relative  flex items-center justify-end ${row.is_dominant_put ? DEPTH_PROFILE_THEME.putDominantBarClass : DEPTH_PROFILE_THEME.putNormalBarClass}`}
                ...
                <div className="w-[50px] h-full flex items-center justify-center relative shrink-0 z-20 bg-[#060606]">
                    ...
                    <span className={`font-mono text-[11px] tracking-tight z-10 ${row.is_spot ? 'text-[#ff3366] font-black bg-[#ff3366]/10 px-0.5 rounded' : row.is_flip ? 'text-[#fbbf24] font-bold' : (isMaxPut || isMaxCall) ? 'text-[#f4f4f5] font-bold' : 'text-[#71717a]'}`}>
                ...
                <div className="flex-1 h-full flex justify-start items-center relative">
                    {hasCall && (
                        <div className={`h-[12px] relative  flex items-center justify-start ${row.is_dominant_call ? DEPTH_PROFILE_THEME.callDominantBarClass : DEPTH_PROFILE_THEME.callNormalBarClass}`}
                    ...
                    {sortedStrikes.map(strike => {
                        ...
                        return (
                            <div key={strike} className="w-full flex justify-end h-[3px] my-[1px] relative">
                                <div className={`h-full ${isActiveViewport ? 'bg-[#a1a1aa]' : 'bg-[#27272a]'}`}
