...
export const DepthProfile: React.FC<Props> = ({ rows, macroVolumeMap, spot }) => {
    ...
    const hasMinimap = macroVolumeMap && Object.keys(macroVolumeMap).length > 0
    let maxVol = 1
    let sortedStrikes: number[] = []

    if (hasMinimap) {
        const vols = Object.values(macroVolumeMap)
                ...
                <div className="flex flex-col w-full relative">
                    {safeRows.map((row) => {
                        ...
                        return (
                            ...
                            >
                                {row.is_spot && (
                                    <div className="absolute left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#ff3366]/10 via-transparent to-transparent z-0 pointer-events-none"></div>
                                )}

                                {row.is_flip && (
                                    <div className="absolute top-[50%] left-0 w-full h-[1px] border-b border-dashed border-[#fbbf24]/50 z-20 pointer-events-none"></div>
                                )}

                                {row.is_spot && (
                                    <div className="absolute top-[50%] left-0 w-full h-[1px] border-b border-dashed border-[#ff3366]/60 z-20 pointer-events-none shadow-[0_0_8px_rgba(255,51,102,0.6)]"></div>
                                )}

                                <div className="flex items-center w-full h-full relative z-10 box-border px-4">

                                    {/* PUT WING - 移除物理 Padding，完全贴合 */}
                                    <div className="flex-1 h-full flex justify-end items-center relative">
                                        {hasPut && (
                                            ...
                                            >
                                                {isMaxPut && (
                                                    <div className="absolute right-0 top-1/2 -translate-y-1/2 bg-[#022c22]/90 border-l border-[#10b981]/40 text-[#34d399] text-[9px] font-black px-[4px] py-[1px] rounded-[1px] leading-none z-10">
                                                        P
                                                    </div>
                                                )}
                                            </div>
                                    ...
                                    {/* CENTER SPINE - 移除 Blur 侵蚀，强化 1px 焊点锚线 */}
                                    <div className="w-[50px] h-full flex items-center justify-center relative shrink-0 z-20 bg-[#060606]">
                                        {hasPut && (
                                            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[1px] h-[16px] bg-[#34d399] z-30 shadow-[0_0_4px_rgba(52,211,153,0.8)]"></div>
                                        )}
                                        ...
                                        {hasCall && (
                                            <div className="absolute right-0 top-1/2 -translate-y-1/2 w-[1px] h-[16px] bg-[#f87171] z-30 shadow-[0_0_4px_rgba(248,113,113,0.8)]"></div>
                                        )}
                                    </div>

                                    {/* CALL WING - 移除物理 Padding，完全贴合 */}
                                    <div className="flex-1 h-full flex justify-start items-center relative">
                                        {hasCall && (
                                            ...
                                            >
                                                {isMaxCall && (
                                                    <div className="absolute left-0 top-1/2 -translate-y-1/2 bg-[#450a0a]/90 border-r border-[#ef4444]/40 text-[#f87171] text-[9px] font-black px-[4px] py-[1px] rounded-[1px] leading-none z-10">
                                                        c
                                                    </div>
                                                )}
                                            </div>
                                ...
                                {row.is_flip && (
                                    <div className="absolute right-1 top-[50%] -translate-y-[120%] z-40 pointer-events-none">
                                        <div className="bg-[#1a1505]/95 border border-[#fbbf24]/80 text-[#fbbf24] text-[9px] font-black px-1.5 py-[1px] rounded-[2px] shadow-[0_2px_4px_rgba(0,0,0,0.5)]">
                                            FLIP
                                        </div>
                                    </div>
                                )}
                                ...
                                {row.is_spot && (
                                    <div className="absolute right-1 top-[50%] -translate-y-[120%] z-50 pointer-events-none">
                                        <div className="bg-[#1a050a]/95 border border-[#ff3366]/80 text-[#ff3366] text-[10px] font-black px-1.5 py-[1px] rounded-[2px] shadow-[0_2px_4px_rgba(0,0,0,0.5)] flex items-center gap-1">
                                            <span>SPOT</span>
                                            <span className="font-mono">{spot ? spot.toFixed(2) : row.strike.toFixed(2)}</span>
                                        </div>
                                    </div>
                                )}
                            </div>
                    ...
                    {sortedStrikes.map(strike => {
                        ...
                        return (
                            <div key={strike} className="w-full flex justify-end h-[3px] my-[1px] relative">
                                <div
                                    className={`h-full transition-all duration-300 ${isActiveViewport ? 'bg-[#a1a1aa] shadow-[0_0_5px_rgba(161,161,170,0.4)]' : 'bg-[#27272a]'}`}
