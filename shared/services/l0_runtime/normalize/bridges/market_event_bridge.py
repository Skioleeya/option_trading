"""Active market-event normalization bridge for L0 runtime ingestion."""
...
from shared.services.l0_runtime.normalize.pipeline.sanitization import (
    CleanQuoteEvent,
    EventType,
    _infer_opt_type,
)
...
def parse_market_event(
    ...
    *,
    ...
    if not symbol or symbol == "\x00":
        return None

    strike = symbol_to_strike.get(symbol)
    if strike is None:
        logger.debug("[MarketEventBridge] Event dropped (unknown symbol): %s", symbol)
        return None
    ...
    try:
        ...
        logger.warning(
            "[MarketEventBridge] Event dropped (invalid event_type): symbol=%s event_type=%s",
            symbol,
            raw_event_type,
        )
        return None
...
def dispatch_depth_event(
    ...
    *,
    ...
    bids, asks = _market_depth_levels(event)
    try:
...
def _market_depth_levels(event: CleanQuoteEvent) -> tuple[list[dict[str, float]], list[dict[str, float]]]:
