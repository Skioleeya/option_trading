...
def _append_arrow_row(arrays: dict[str, list], event: dict[str, object]) -> None:
...
def _build_arrow_arrays(events: list[dict[str, object]]) -> dict[str, list]:
...
class EventLayoutRegistry:
    ...
    @classmethod
    def decode_event(cls, layout: EventLayout, data: bytes) -> dict[str, object]:
...
class RustBridge:
        ...
        for variant in path_variants:
            for fallback_layout in EventLayoutRegistry.supported_layouts():
                ...
                try:
                    ...
                    logger.warning("[RustBridge] PERMISSION DENIED for variant '%s': %s", variant, exc)
                    break
    ...
    def to_arrow_batch(self, events: list[dict[str, object]]) -> pa.RecordBatch | None:
        if not events:
            return None
